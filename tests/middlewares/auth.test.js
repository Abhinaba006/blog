...
describe('auth middleware', () => {
  ...
  test('returns 401 JSON for /api URLs when auth fails', async () => {
    const req = { cookies: { token: 'bad-token' }, originalUrl: '/api/users' }
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'Please login or sign up' })
    expect(res.render).not.toHaveBeenCalled()
    expect(next).not.toHaveBeenCalled()
  })
